...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!supabase) {
    return apiError("setup_required", "Supabase service role is required to cancel a deal.", 503);
  }
  ...
  return apiOk({ dealId: id, status: "cancelled", auditEvent: "deal_cancelled" });
}
